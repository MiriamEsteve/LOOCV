...
class Peyrache:
    def __init__(self, matrix, x, y): # x = Conjunto de variables que siempre van a estar; model = "BCC_output_oriented"
        self.VARCol = x
    ...
    def fit_Peyrache(self):

        return

    def Peyrache_model(self):
        # create one model instance, with a name
        m = Model(name='Peyrache')

        # by default, all variables in Docplex have a lower bound of 0 and infinite upper bound
        u = [{i: m.continuous_var(name="u_{0}".format(i)) for i in range(self.s)} * self.N] # Cada DMU (n) tiene s componentes de u
        v = {i: m.continuous_var(name="v_{0}".format(i)) for i in range(self.mVAR * self.N)} # Cada DMU (n) tiene p1 componentes de v
        w = {i: m.continuous_var(name="w_{0}".format(i)) for i in range(self.mnoVAR * self.N)} # Cada DMU (n) tiene p1 + p2 = p componentes de w

        pi = {i: m.binary_var(name="pi_{0}".format(i)) for i in range(self.s * self.N)}
        rho = {i: m.binary_var(name="rho_{0}".format(i)) for i in range(self.mnoVAR * self.N)}


        # Constrain 1.1
        m.add_constraint(m.sum(v[n] for n in range(self.mVAR)) == 1)  # sum(lambda) = 1
    ...
    def DEA(self, matrix):
        ...
        for i in range(len(matrix)):
            if self.model == "BCC_output_oriented":
